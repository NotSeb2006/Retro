...
AddEventHandler('food:Fruit', function()
  currentValues["hunger"] = currentValues["hunger"] + 13
...
Citizen.CreateThread(function()
  while true do
    ...
    if currentValues["oxy"] > 0 and IsPedSwimmingUnderWater(plyPed) then
    ...
    if counter == 0 then
      ...
      if valueChanged then
        ...
        exports["np-ui"]:sendAppEvent("hud", {
            crosshairShow = currentValues["crosshair"], -- lmao
...
AddEventHandler("baseevents:enteredVehicle", function(pVehicle)
  ...
  if (IsThisModelAPlane(vehicleModel) or IsThisModelAHeli(vehicleModel)) then
    enterVehicleSetTurbulence(true)
  else
    enterVehicleSetTurbulence(false)
  end
end) 
...
function enterVehicleSetTurbulence(shouldInact)
  TriggerEvent("keys:cannotOperate", false)
  local myJob = exports["isPed"]:isPed("myJob")
  if myJob == "police" or myJob == "ems" then return end

  if not IsPedInAnyVehicle(PlayerPedId(), false) then return end

  local pedVehicle = GetVehiclePedIsUsing(PlayerPedId())
  local turbulence = 0.0
  local cannotOperate = false
  local hasLicense = exports["np-gov"]:HasLicense("pilot_license")

  if shouldInact and not hasLicense then
    turbulence = turbulenceMin
    cannotOperate = true
  end

  if pedVehicle == nil or pedVehicle == 0 then return end

  if IsThisModelAPlane(GetEntityModel(pedVehicle)) then

    TriggerEvent("keys:cannotOperate", cannotOperate)
    SetVehicleEngineOn(pedVehicle,0,1,1)
    SetVehicleUndriveable(pedVehicle,true)

  elseif IsThisModelAHeli(GetEntityModel(pedVehicle)) then
    if not hasLicense then 
      setHeliTurbulence = true
    end
    SetHeliTurbulenceScalar(pedVehicle,turbulence)
  end
end

Citizen.CreateThread(function()
  while true do
    if setHeliTurbulence then
      local ped = PlayerPedId()
      if not IsPedInAnyVehicle(ped, false) then setHeliTurbulence = false end
      local pedVehicle = GetVehiclePedIsUsing(ped)
      local height = GetEntityHeightAboveGround(pedVehicle)
      if height >= 10.0 and IsThisModelAHeli(GetEntityModel(pedVehicle)) then
