...
AddEventHandler("K9:Create", function(dogType)
		...
		DecorSetBool(spawned_entity, 'ScriptedPed', true)
...
AddEventHandler("K9:Vehicle", function()
	...
	if IsPedInAnyVehicle(current_dog, false) and imDead == 0 then
		...
		SetEntityCoords(current_dog, xnew, ynew, plyPos.z)
		--TriggerEvent("K9:Follow")
		...
		if CheckVehicleRestriction(vehicle) == true and imDead == 0 then
		    ...
		    for i=1,GetVehicleMaxNumberOfPassengers(vehicle) do
		      if IsVehicleSeatFree(vehicle,i) then
		        ...
		        SetPedIntoVehicle(current_dog,vehicle,i)
		        dogvehicled = true				        
		        return true
		      end
		    end
...
Citizen.CreateThread(function()
	while true do
		...
		if current_dog == 0 then
			...
			-- Attacking Key --
			if IsPlayerFreeAiming(PlayerId()) and IsControlJustPressed(1, 243) and not IsPedSittingInAnyVehicle(PlayerPedId()) then
			...
			if IsControlPressed(1, 243) and IsControlPressed(1, 19) and not IsPedSittingInAnyVehicle(PlayerPedId()) then
...
AddEventHandler("K9:HuntAccepted", function(other_ped, other_coords, other_distance)

	ClearPedTasks(current_dog)
	hunt_coords = other_coords
	if hunting == false and other_distance < 230 then
