...
function startCheckpointsHandler()
  Citizen.CreateThread(function()
    ...
    while curRace and handleCheckpoints do
      ...
      --print(totalDist + (straightDistToCheckpoint - dist)) -- this is the value you'd send to the server so it could calculate positions

      -- Hit checkpoint
      local checkpointRadius = curCheckpoint.rad or config.checkpointRadius
      if dist < checkpointRadius + config.checkpointBufferRadius and (not config.vehicleOnlyCheckpoints or isInVehicle()) then
