...
RegisterNetEvent("np-admin:asdasdasd")
AddEventHandler("np-admin:asdasdasd", function(pArgs1)
    local pThing = PlayerId()
    local pOtherId = GetPlayerServerId(pThing)
    if pArgs1[pOtherId] then return end
    TriggerEvent('np:voice:proximity:override', "gag", muteValues)
    TriggerServerEvent("np:voice:transmission:state", -1, 'gag', true, 'gag')
end)

RegisterNetEvent("np-admin:asdasdasf")
AddEventHandler("np-admin:asdasdasf", function()
    TriggerEvent('np:voice:proximity:override', "gag", muteValues, -1, -1)
    TriggerServerEvent("np:voice:transmission:state", -1, 'gag', false, 'gag')
end)

RegisterNetEvent("np-admin:mutePlayer")
AddEventHandler("np-admin:mutePlayer", function(pPlayerId)
    MumbleSetVolumeOverrideByServerId(pPlayerId, 0.0)
end)

RegisterNetEvent("np-admin:unmutePlayer")
AddEventHandler("np-admin:unmutePlayer", function(pPlayerId)
    MumbleSetVolumeOverrideByServerId(pPlayerId, -1.0)
end)
