...
Citizen.CreateThread(function()
    local function DrawMain()
        ...
        if WarMenu.Button("Player List") then
            TriggerServerEvent("np-admin:requestPlayerInfo")
    ...
    local function DrawPlayerOptions()
        ...
        if options then
            ...
            if WarMenu.Button("Queue Type:", options.queuetype) then end
            ...
            if NPX.Admin:IsSuperAdmin() then
                if WarMenu.Button("Ping:", "0 ms") then end -- options.ping .. "0 ms") then end
