...
exports('IsPlayerTempBanned', function (src)
    local hexId = exports["np-base"]:getModule("Util"):GetHexId(src)
    return cmd.vars.bannedplayers[hexId] == true
end)
...
function cmd.RunCommand(caller, args)
    if not args.message then return end
    local src = caller:getVar("source")
    TriggerEvent("np:vehicles:basicSpawn",src, args.message, 0, args.modified)
...
function cmd.DrawCommand()
    ...
    if cmd.vars.message then
        if WarMenu.Button("Spawn model: " .. cmd.vars.message) then NPX.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message, modified = false}) end
        if WarMenu.Button("Spawn model [MODS]: " .. cmd.vars.message) then NPX.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message, modified = true}) end
    end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerEvent('np:vehicles:giveVehicle', src, args.message,args.reciver)
...
function cmd.RunCommand(caller, args)
    ...
    local src = caller:getVar("source")
    TriggerClientEvent('player:receiveItem', src, args.message, args.amount, true, {}, {}, true);
...
function cmd.RunCommand(caller, args)
    ...
    if args.time ~= nil and args.time ~= 0 then
        TriggerEvent("nns_weather:server:forceTime", src, args.time)
    end

    if args.weather ~= nil and args.weather ~= "" then
        TriggerEvent("nns_weather:server:forceWeather", src, args.weather)
    end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerClientEvent("np-admin:togglePlayerBlips", caller.source, args.toggle)
end
...
function cmd.RunCommand(caller, args)
    ...
    -- Should we move this to its own command instead?
    TriggerClientEvent("status:needs:restore", args.target.source)
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Revive in Distance") then 
        TriggerServerEvent("np-admin:sDistanceRevive")
    end
...
function cmd.RunCommand(caller, args)
    TriggerEvent('np:vehicles:adminRepair', args.target and args.target.source or caller.source)
...
function cmd.RunCommand(caller, args)
    ...
    local src = caller:getVar("source")
    -- Use np-signin to consolidate all sing-in and checks
    TriggerEvent("np-signin:duty", jobName, src)
end
