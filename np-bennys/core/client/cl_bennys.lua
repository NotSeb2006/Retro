...
local bennysLocations = {
    ...
    ["bennyspaletopd"] = {
      pos = vector3(-448.44, 5994.54, 31.35),
      heading = 270.3,
      access = 'police'
    },
    ["bennyspillbox"] = {
    ...
    ["bennyshubcatalog"] = {
        pos = vector3(-29.19, -1058.64, 28.27),
        heading = 250.0,
        employed = true,
        -- repairOnly = true
    },
    ["bennyshubcatalog2"] = {
        pos = vector3(-27.97, -1055.39, 28.27),
        heading = 250.0,
        employed = true,
        -- repairOnly = true
    },
    ["bennyshubcatalog3"] = {
        pos = vector3(-31.37, -1048.59, 28.27),
        heading = 250.0,
        employed = true,
        -- repairOnly = true
    },
    ["bennyshubrepairs"] = {
        pos = vector3(-39.19, -1053.03, 28.27),
        heading = 160.0,
        repairOnly = true
    },
    ["bennysflightschool"] = {
...
function AttemptPurchase(purchaseType, upgradeLevel)
    ...
    TriggerServerEvent("np-bennys:attemptPurchase", cheap, purchaseType, upgradeLevel)
...
function ExitBennys()
    ...
    saveVehicle()
    if isAtEmployedBennys then
        print("Reset vehicle to default")
    ...
    TriggerEvent("vehicle:leftBennys", isAtEmployedBennys, isAtRepairsOnlyBennys, devmode)
...
function enterLocation(pBennys, needsRepair)
    ...
    if devmode then
        ...
        if (vehclass == 18 and (bennysAccess == "police")) or (needsRepair) then
            freezeVehicle(plyVeh, pBennys)
        elseif (vehclass ~= 18 and (bennysAccess ~= "police" or bennysAccess ~= "ems")) or (needsRepair) then
            freezeVehicle(plyVeh, pBennys)
        end
    end
...
function getClosestBennys()
    ...
    for bennys, v in pairs(bennysLocations) do
        ...
        if dist == -1 or newdist < dist then
            ...
            isAtEmployedBennys = closestBennys.employed or false
