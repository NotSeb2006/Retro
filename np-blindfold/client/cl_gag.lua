...
AddEventHandler("gag:toggle", function(pArgs, pEntity)
  ...
  isGagged = exports["np-flags"]:HasPedFlag(tPed, "isGagged")
  ...
  local gagtask = exports["np-taskbar"]:taskBar(time, string)

  if gagtask == 100 then
    if isGagged then
      exports['np-flags']:SetPedFlag(tPed, 'isGagged', false)
    else
      exports['np-flags']:SetPedFlag(tPed, 'isGagged', true)
    end
  end
