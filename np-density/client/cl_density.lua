...
Citizen.CreateThread(function ()
  local config = exports['np-config']:GetModuleConfig('main')['defaultDensity']

  defaultDensity = (config == nil or type(config) ~= "number") and 0.8 or config
end)

AddEventHandler('np-config:configLoaded', function (pId, pConfig)
  if (pId ~= 'main') then return end

  local config = pConfig['defaultDensity']
