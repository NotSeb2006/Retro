...
AddEventHandler("np-editor:buildName", function(name,objects)
    if objects then
...
function addItemToCurrent(data)
        ...
        for k,v in pairs(CURRENT_ENTITY[CURRENT_NAME]) do
            if DoesEntityExist(v) then
                ...
                if CURRENT_EDITED[v] then
                    if #(GetEntityCoords(v, true, true) - CURRENT_OBJECTS[CURRENT_NAME][k].coords) > 0.2  then
                        shouldChange = true
                    end
                end
                ...
                local object = {
                    ...
                    quat = vector4(x,y,z,w),
                    realName = placedName,
                    id = CURRENT_OBJECTS[CURRENT_NAME][k].id,
