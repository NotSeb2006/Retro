...
function addSpecial(modelNames)
    local specialAdded = 1
    for k,v in pairs(modelNames) do
        local model = GetHashKey(v)
        if IsModelValid(model) then
            if PLACED_LIST[model] then
                --print('Overlap Special: ' .. model .. ' | ' .. v)
            else
                if HASH_MODELS[added] == nil then
                    HASH_MODELS[added] = {
                        "special",
                        {[specialAdded] = {model,v}} 
                    }
                else
                    HASH_MODELS[added][2][k] = {model,v}
                end
...
function readyModels(force)
    ...
    if CURRENT_SPECIAL ~= nil then
        addSpecial(CURRENT_SPECIAL)
    end
end
