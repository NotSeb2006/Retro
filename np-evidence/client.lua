...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if not IsPedArmed(ped, 7) then
      ...
      if IsPedShooting(ped) and not excludedWeapons[hash] then
...
AddEventHandler('nns_weather:weatherHasChanged', function(pWeatherProgression)
  currentWeather = pWeatherProgression.weathers[1]
end)
...
RegisterNetEvent('evidence:dna')
AddEventHandler('evidence:dna', function(dnaType)

    local ped = PlayerPedId()
    local cid = exports["isPed"]:isPed("cid")

    local identifier = dnaType.."-"..cid
...
RegisterNetEvent('evidence:drugs')
AddEventHandler('evidence:drugs', function(drugType)
  local cid = exports["isPed"]:isPed("cid")

  local identifier = drugType.."-"..cid..math.floor((math.random() * 100000))
...
AddEventHandler('evidence:bleeding', function(corpse)


    if inveh and not corpse then
      return
    end

    if not WaterTest() or corpse then
          ...
          if finished == 100 then
            ...
            --  PickUpItem(Zone,scannedEvidence[closestID]["meta"]["identifier"],scannedEvidence[closestID]["meta"]["evidenceType"],scannedEvidence[closestID]["meta"]["other"],"casing")
              ...
              for _, id in pairs(closestIDs) do
                ...
                local EvidenceIdent = scannedEvidence[id]["meta"]["identifier"]
                ...
                elseif evType == "vehiclefragment" then
                  EvidenceIdent = scannedEvidence[id]["meta"]["other"]
                  evItem = "np_evidence_marker_purple"
                end                
                local pickedUpKey = EvidenceIdent .. "_" .. evType
                if not pickedUp[pickedUpKey] then
                  pickedUp[pickedUpKey] = true
                  PickUpItem(EvidenceIdent,evType,scannedEvidence[id]["meta"]["other"], evItem)
                  Wait(500)
                end
              end
