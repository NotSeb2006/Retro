...
local function processSettings(settings, forPaleto)
  ...
  if not farmersMarketDynamicDisplayEnabled then return end
  for _, conf in pairs(duiReplaces) do
    local url = (isDayTime(true) and settings[conf.id]) and settings[conf.id].image or "https://i.imgur.com/ie675vt.png"
...
RegisterUICallback("np-ui:farmersmarket:craftItems", function(data, cb)
  ...
  if not exports["np-inventory"]:hasEnoughOfItem(ingredient, qty) then
    TriggerEvent("DoLongHudText", "Not enough ingredients (" .. ingredient .. ")", 2)
    return
  end
...
RegisterUICallback("np-ui:getFarmersItems", function(data, cb)
  local result = RPC.execute("np-farmersmarket:getFarmersItems")
