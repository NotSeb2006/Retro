...
function IsGalleryEmployee()
  ...
  local jobAccess = RPC.execute("IsEmployedAtBusiness", { character = { id = characterId }, business = { id = "gallery" } })
...
AddEventHandler("np-gallery:sellGems", function()
  ...
  RPC.execute("np-gallery:sellGemsFromNPC")
end)
...
RegisterUICallback("np-ui:gallery:pictureReplaceAction", function(data, cb)
  ...
  RPC.execute("np-gallery:savePictureReplace", room, id, url)
end)
...
AddEventHandler("np-polyzone:enter", function(zone)
  ...
  RPC.execute("np-gallery:getRoomPaintingSettings")
end)
...
--   for i = 1, 12, 1 do
--     RPC.execute("np-gallery:savePictureReplace", 6, i, "https://i.imgur.com/M95SXa2.png")
--   end
-- end, false)
