...
function enterEdit(housingName)
    ...
    if playerInRangeOfProperty(propertyID) then 
        ...
        local availableEditOptions = {
            ...
            crafting = ((edit.crafting and hasProgression) or (Housing.max[houseCat].publicCrafting and isPublic)) and isOwner,
...
function switchBenchInventory(isBench)
    ...
    if isBench then
        local isPublic = exports["np-config"]:IsPublic()
        if not hasCorrectFlags("canHaveCrafting",Housing.currentlyEditing) and not isPublic then return false, TriggerEvent("DoLongHudText","This is not available for the property.",2) end
