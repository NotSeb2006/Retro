local config = {
  -- FASTLANE YEETED
  -- fastlane = {
  --   carSpawns = {
  --     {
  --       ["coords"] = vector4(-787.03, -243.37, 36.32, 85.14),
  --       ["info"] = "Car Spot 1",
  --       ["model"] = "r1",
  --     },
  --     {
  --       ["coords"] = vector4(-790.26, -237.81, 36.32, 95.3),
  --       ["info"] = "Car Spot 2",
  --       ["model"] = "fnfrx7",
  --     },
  --     {
  --       ["coords"] = vector4(-792.6, -233.09, 36.32, 100.49),
  --       ["info"] = "Car Spot 3",
  --       ["model"] = "r1",
  --     },
  --     {
  --       ["coords"] = vector4(-795.7, -228.24, 36.32, 94.29),
  --       ["info"] = "Car Spot 4",
  --       ["model"] = "fnf4r34",
  --     },
  --     {
  --       ["coords"] = vector4(-783.25, -223.68, 36.56, 171.93),
  --       ["info"] = "Car Spot 5",
  --       ["model"] = "gt63",
  --     },
  --   },
  --   polyZone = {
  --     coords = {
  --       vector2(-833.13543701172, -183.4736328125),
  --       vector2(-858.15698242188, -197.16130065918),
  --       vector2(-825.49865722656, -254.89807128906),
  --       vector2(-815.40686035156, -272.62399291992),
  --       vector2(-818.42425537109, -283.65115356445),
  --       vector2(-774.29351806641, -302.78747558594),
  --       vector2(-770.70660400391, -290.49176025391),
  --       vector2(-775.77282714844, -272.02325439453),
  --       vector2(-746.96704101563, -260.69543457031),
  --       vector2(-736.48358154297, -255.03248596191),
  --       vector2(-795.85998535156, -159.32734680176),
  --     },
  --     options = {
  --       name = "fastlane",
  --       minZ = 30.5,
  --       maxZ = 50.5,
  --       gridDivisions = 25,
  --       debugGrid = false,
  --     },
  --   },
  --   testDriveSpawnPoint = vector4(-789.3, -196.07, 37.1, 298.95),
  -- },
  pdm = {
    carSpawns = {
      {
        ["coords"] = vector4(-38.25, -1104.18, 26.43, 14.46),
        ["info"] = "Car Spot 1",
        ["model"] = "gauntlet",
      },
      {
        ["coords"] = vector4(-36.36, -1097.3, 26.43, 109.4),
        ["info"] = "Car Spot 2",
        ["model"] = "dubsta3",
      },
      {
        ["coords"] = vector4(-43.11, -1095.02, 26.43, 67.77),
        ["info"] = "Car Spot 3",
        ["model"] = "landstalker",
      },
      {
        ["coords"] = vector4(-50.45, -1092.66, 26.43, 116.33),
        ["info"] = "Car Spot 4",
        ["model"] = "bobcatxl",
      },
      {
        ["coords"] = vector4(-56.24, -1094.33, 26.43, 157.08),
        ["info"] = "Car Spot 5",
        ["model"] = "surfer",
      },
      {
        ["coords"] = vector4(-49.73, -1098.63, 26.43, 240.99),
        ["info"] = "Car Spot 6",
        ["model"] = "glendale",
      },
      {
        ["coords"] = vector4(-45.58, -1101.4, 26.43, 287.3),
        ["info"] = "Car Spot 7",
        ["model"] = "washington",
      },
    },
    polyZone = {
      coords = {
        vector2(-17.224317550659, -1125.9611816406),
        vector2(-70.010810852051, -1128.2976074219),
        vector2(-76.185691833496, -1127.8470458984),
        vector2(-79.25121307373, -1123.7583007813),
        vector2(-79.670585632324, -1118.4036865234),
        vector2(-59.549613952637, -1063.388671875),
        vector2(-1.2465063333511, -1081.7679443359),
      },
      options = {
        name = "pdm",
        minZ = 0,
        maxZ = 40.5,
        gridDivisions = 25,
        debugGrid = false,
      },
    },
    testDriveSpawnPoint = vector4(-51.51, -1077.96, 26.92, 80.0),
  },
  tuner = {
    carSpawns = {
      {
        ["coords"] = vector4(954.1, -954.0, 39.51, 95.0),
        ["info"] = "Car Spot 1",
        ["model"] = "zx10",
      },
      {
        ["coords"] = vector4(954.1, -951.0, 39.51, 95.0),
        ["info"] = "Car Spot 2",
        ["model"] = "m4",
      },
      {
        ["coords"] = vector4(954.1, -948.0, 39.51, 95.0),
        ["info"] = "Car Spot 3",
        ["model"] = "dc5",
      },
      {
        ["coords"] = vector4(953.1, -945.0, 39.51, 95.0),
        ["info"] = "Car Spot 4",
        ["model"] = "raid",
      },
      {
        ["coords"] = vector4(949.87, -940.38, 39.51, 171.34),
        ["info"] = "Car Spot 5",
        ["model"] = "lwlp670",
      },
      {
        ["coords"] = vector4(946.03, -940.72, 39.51, 184.55),
        ["info"] = "Car Spot 6",
        ["model"] = "gt2rwb",
      },
      {
        ["coords"] = vector4(942.07, -942.46, 39.51, 211.21),
        ["info"] = "Car Spot 7",
        ["model"] = "bdragon",
      },
    },
    polyZone = {
      coords = {
        vector2(901.94128417969, -944.42016601563),
        vector2(901.19378662109, -988.61682128906),
        vector2(937.08911132813, -986.65545654297),
        vector2(938.18969726563, -995.44683837891),
        vector2(949.73962402344, -995.05004882813),
        vector2(953.41033935547, -995.00726318359),
        vector2(952.57049560547, -985.20684814453),
        vector2(963.91479492188, -984.59216308594),
        vector2(964.98822021484, -983.99652099609),
        vector2(961.36340332031, -933.67346191406),
        vector2(926.74658203125, -933.97003173828),
      },
      options = {
        name = "tuner",
        minZ = 0,
        maxZ = 55,
        gridDivisions = 25,
        debugGrid = false,
      },
    },
    testDriveSpawnPoint = vector4(935.61, -990.8, 38.0, 252.4),
  },
}

function getConfigFor(showroom)
  return config[showroom]
end
