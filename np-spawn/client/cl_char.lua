...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    events:Trigger("np-base:fetchPlayerCharacters", nil, function(data)
        ...
        local selectedCid = nil --RPCEvent {1001,1003}
        if GetResourceState("np-queue") == "started" then
            selectedCid = RPC.execute("np-queue:getAllowedCharacters")
        end
        for _=1, exports["np-config"]:GetModuleConfig("main").characterLimit do
            ...
            if data[_] ~= nil then 
                ...
                if selectedCid ~= nil then
                    local hasPassed = false
                    for k,v in pairs(selectedCid) do
                        if cid == v then 
                            hasPassed = true
                        end
                    end
                    if not hasPassed then isBlocked = true end
                end
            ...
            Login.RequestModel(cModelHash, function(loaded, model, modelHash)
                if loaded and not isBlocked then
                    local newPed = nil
                    ...
                    if character ~= nil then
                        ...
                        if success then
                            ...
                            local errorhash = GetHashKey("np_m_character_select")
                            newPed = CreatePed(3, errorhash, Login.spawnLoc[_].x, Login.spawnLoc[_].y, Login.spawnLoc[_].z, 0.72, false, false)
