...
function Spawn.getCrash(crashPos)
	local spawn = nil

	if crashPos then
		local vec3 = vector3(crashPos.x,crashPos.y,crashPos.z)
		local newPos = vector4(crashPos.x,crashPos.y,crashPos.z,0.0)
		local canUse = true
		...
		if canUse then
			spawn = { ["pos"] = newPos, ['info'] = 'Crash Location'}
			Spawn.Crash = newPos
		end
	end
