...
AddEventHandler('spikes:watch', function(x,y,z,obj)
	...
	while timer < 500 do
		...
		if driverPed then
			...
			local skip = false
			if #(pos - leftfront) < 1.5 and not IsVehicleTyreBurst(veh,0,true) then
				if IsEntityTouchingEntity(veh,GetClosestObjectOfType(x,y,z,5.0,spike,0,0,0)) then
					SetVehicleTyreBurst(veh, 0, true, 1000.0)
				end
			end
			if #(pos - rightfront) < 1.5 and not IsVehicleTyreBurst(veh,1,true) then
				if IsEntityTouchingEntity(veh,GetClosestObjectOfType(x,y,z,5.0,spike,0,0,0)) then
					SetVehicleTyreBurst(veh, 1, true, 1000.0)
				end
			end
			if #(pos - leftback) < 1.5 and not IsVehicleTyreBurst(veh,4,true) then
				if IsEntityTouchingEntity(veh,GetClosestObjectOfType(x,y,z,5.0,spike,0,0,0)) then
					SetVehicleTyreBurst(veh, 2, true, 1000.0)
					SetVehicleTyreBurst(veh, 4, true, 1000.0)	
				end		      		
			end
			if #(pos - rightback) < 1.5 and not IsVehicleTyreBurst(veh,5,true) then
				if IsEntityTouchingEntity(veh,GetClosestObjectOfType(x,y,z,5.0,spike,0,0,0)) then
